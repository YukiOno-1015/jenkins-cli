...
def call(Map args = [:]) {
    ...
    if (!sshCred) {
        ...
        echo "repositoryConfig から認証情報を解決しました: ${sshCred}"
    }
    ...
    echo "使用する SSH 認証情報 ID: ${sshCred}"
    if (rawBranch != branch) {
        echo "ブランチ名を正規化しました: '${rawBranch}' -> '${branch}'"
    }
    ...
    sshagent(credentials: [sshCred]) {
                sh """#!/bin/bash
                    ...
                    echo "known_hosts に ${knownHost} を追加します（rsa/ecdsa/ed25519）"
                    ...
                    # Git クローン
                    echo "リポジトリをクローンします: ${repoUrl}（ブランチ: ${branch}）"

                    # GitHub への SSH 認証を事前確認（接続成功時も終了コードは 0/1 の可能性があるためログ判定）
                    authOutput="\$(ssh -T git@${knownHost} 2>&1 || true)"
                    echo "SSH 認証事前確認の出力: \${authOutput}"
                    if echo "\${authOutput}" | grep -qi "Permission denied (publickey)"; then
                        echo "ERROR: SSH 認証に失敗しました（Jenkins credentials ID: ${sshCred}）"
                        echo "Jenkins の秘密鍵と GitHub 側のリポジトリアクセス権限を確認してください。"
                        exit 128
                    fi
